...
def mask_california(lon_lat,X):
    #x = cf.COASTLINE.with_scale('10m').geometries()
    x = cf.NaturalEarthFeature(category='physical',name='land',scale='10m').geometries()
    for idx, i in enumerate(x):
        if idx == 0:
            Cali = shape(i.geoms[3])
    ...
    mask = mask.reshape((lon, lat))
    mask = np.repeat(mask[:,:,np.newaxis], repeats=step, axis=2)
    X = np.where(mask,X,np.nan)dates, data, power_cal = spacial('SA_Solar_Distribution', '1/7/2019', '1/7/2020', '24', 'SA_Solar_Distribution', 40)

    return X


#L, X, dates = spacial('California', '16/08/2020', '12/11/2020', 3)
L, X, dates = spacial('Australia_LowRes', '1/07/2019', '1/07/2020', 3)
...
plt.plot(dates[::8], area[::8] * 80**2 / 7692024 * 100)
plt.ylabel('Percent of Land in California Affected (%)')
plt.xlim(left=dt.datetime(2019,7,1,12),right=dt.datetime(2020,7,1,12))
#plt.xticks([dt.datetime(2020,1,1,12),dt.datetime(2020,7,1,12),dt.datetime(2021,1,1,12)])
#plt.ylim(bottom=0,top=6)
#plt.savefig('California_Land_Percent.png', dpi=600)
plt.show()
