...
def spacial(dir, start_date, end_date, step, locs):
    ...
    for i in range(len(locs_names))[::]:
        ...
        ax[i].axvspan(xmin=dt.datetime(2020,6,12,12),xmax=dt.datetime(2020,6,24,12),facecolor='tab:purple',alpha=0.25   )
        if i != len(locs_names)-1:
            dates = [dt.datetime(2020, 5, 1, 12), dt.datetime(2020, 8, 1, 12)]
            #dates = [dt.datetime(2020, 1, 1, 12), dt.datetime(2020, 7, 2, 12), dt.datetime(2020, 12, 31, 12)]
            ax[i].set_xticks(dates)
            ax[i].xaxis.set_visible(False)
        else:
            dates = [dt.datetime(2020, 5, 1, 12),dt.datetime(2020, 8, 1, 12)]#,dt.datetime(2020,12,31,12)]
            #dates = [dt.datetime(2020, 1, 1, 12), dt.datetime(2020, 7, 2, 12), dt.datetime(2020, 12, 31, 12)]
...
spacial('CaribbeanLocs','12/06/2020','24/06/2020','24','CaribbeanLocs')
plt.show()
