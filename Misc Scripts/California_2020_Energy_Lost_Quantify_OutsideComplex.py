...
def spacial(dir, start_date, end_date, step, locs):
    ...
    dates_remove = pd.date_range(start='08/16/2020',end='11/12/2020',freq=str(step)+'H').values.ravel()
    dates = dates[~np.isin(dates,dates_remove)]
    ...
    for i in range(len(locs_names)):
        energy_loss_sum = [data[locs_lon_arg[i],locs_lat_arg[i],j] for j in dates_idx]
        energy_loss_sum_C = np.interp(np.linspace(0,1,num=len(dates_idx)*3), np.linspace(0,1,num=len(dates_idx)),energy_loss_sum)
        ax[i].plot(dates_of_data_list[dates_idx],energy_loss_sum,color=colours[i])
        print(locs_names[i])
        print(np.nanmean(energy_loss_sum_C))
        #print(np.nanmean(energy_loss_sum))
        #ax[i].set_ylim(top=0,bottom=-20)
    ...
    fig.text(0.02, 0.5, 'Electrical Power Loss (W/Wm$^{-2}$)', va='center', rotation='vertical')

    return



spacial('California','1/01/2020','31/12/2020','3','CaliforniaLocs')
plt.show()
