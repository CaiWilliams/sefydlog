...
def spacial(dir, start_date, end_date, step, lat, lon):
    ...
    plt.plot(dates,data[dates_idx])
    print('Average', np.average(data[dates_idx]))
    print('Maximum loss',np.min(data[dates_idx]))
    plt.show()
    # ax = ax.flatten()
    ...
    #             ax[i].text(x=0.5, y=-0.17, s='+' + str(int(step * i / 24)) + ' Days', transform=ax[i].transAxes, fontsize=9, ha='center')
