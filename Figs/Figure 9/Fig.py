...
def spacial_years_mean(dir, start_date, end_date,step):
    step = step * 24
    ...
    dates = pd.date_range(start_date,end_date,freq='365D').values.ravel()
    ...
    for i in range(len(ax)):
        if i >= len(dates_idx) or i == 0:
            fig.delaxes(ax[i])
        else:
            if i == 0:
                cs = ax[i].contourf(longitudes, latitudes, np.mean(data[:, :, 0:dates_idx[i]], axis=2), transform=proj, levels=l, cmap='inferno_r')
            else:
                cs = ax[i].contourf(longitudes, latitudes, np.mean(data[:, :, dates_idx[i-1]:dates_idx[i]:8],axis=2), transform=proj, levels=l, cmap='inferno_r')
                ax[i].text(x=0.5, y=-0.15, s=str(2002 + int(step * i / 24/365)), transform=ax[i].transAxes, fontsize=9, ha='center')
            ax[i].coastlines(color='white')
            ax[i].plot(116.387985, 39.996353, transform=ccrs.PlateCarree(), marker='o', markersize=3, c='white')
    ...
    cbar = fig.colorbar(cs,cax=cbar_ax,orientation='vertical',label='Mean Electrical Power Loss (Wm$^{-2}$)',ticks=[-10,-20,-30,-40,-50])
    return
 
spacial_years_mean('Beijing','1/01/2003','31/12/2020',1095)
