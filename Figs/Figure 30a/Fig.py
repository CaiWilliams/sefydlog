...
def calc_multiyear(locations, start_date, end_date,noct):
    ...
    for location in locations:
        polluted = []
        for year in years:
            pristine_dir = 'FullRes_3hr_PERC_' + location + '_pristine_tilt_angle__NOCT_' + str(noct) + '_' + str(year)
            polluted_dir = 'FullRes_3hr_PERC_' + location + '_tilt_angle__NOCT_' + str(noct) + '_' + str(year)
...
def spacial(dir, start_date, end_date, step):
    ...
    for i in range(len(ax)):
        if i >= len(dates_idx):
            ...
            l = np.around(np.arange(-40,1,1),0)
    ...
    cbar = fig.colorbar(cs,cax=cbar_ax,orientation='vertical',label='Electrical Power Loss (Wm$^{-2}$)', ticks=[-0,-5,-10,-15,-20,-25,-30,-35,-40])

    return fig,ax


def spacial2(dir, start_date, end_date, step,fig,ax):
    start_date = pd.to_datetime(start_date, dayfirst=True, )
    end_date = pd.to_datetime(end_date, dayfirst=True)
    ...
    for i in range(len(ax)):
        if i >= len(dates_idx):
            ...
            if np.any(data[:, :, dates_idx[i]]) is not float:
                ...
                try:
                    print(data[x[0], x[1], dates_idx[i]])
                    print(subfiles[(subfiles['Latitude'] == latitudes_unique[x[0]]) & (
                                subfiles['Longitude'] == longitudes_unique[x[1]])].values[0])
                except:
                    print('no')
            cs = ax[i].contourf(longitudes_unique, latitudes_unique, data[:, :, dates_idx[i]],
                                transform=ccrs.PlateCarree(), levels=l, cmap='inferno_r')
            if i != 0 and i < 4:
                ax[i].set_title('+' + str(int(step * i / 24)) + ' Days', fontsize=9, pad=3)
            elif i >= 4 and i < len(dates_idx):
                ax[i].text(x=0.5, y=-0.15, s='+' + str(int(step * i / 24)) + ' Days', transform=ax[i].transAxes,
    ...
    cbar_ax = fig.add_axes([0.85, 0.15, 0.01, 0.7])
    cbar = fig.colorbar(cs, cax=cbar_ax, orientation='vertical', label='Electrical Power Loss (Wm$^{-2}$)',ticks=[-0,-5,-10,-15,-20,-25,-30,-35,-40])
    fig.text(0.05, 0.65, '2008-5-23', va='center', rotation='vertical', fontsize=9)
    fig.text(0.8, 0.35, '2008-5-30', va='center', rotation='vertical', fontsize=9)
    return
 
fig,ax = spacial('EuropeNAmerica_LatitudeLine','23/05/2008','30/05/2008',24)
spacial2('EuropeNAmerica_LatitudeLine','23/05/2008','30/05/2008',24,fig,ax)
#plt.show()
